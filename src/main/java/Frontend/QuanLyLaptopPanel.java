...
public class QuanLyLaptopPanel extends JPanel {
    private LaptopBUS laptopBUS = new LaptopBUS();
    private JTable table;
    private DefaultTableModel tableModel;

    public QuanLyLaptopPanel() {
        // Sử dụng MigLayout để quản lý khoảng cách (insets 20)
    ...
    private void initComponents() {
        // 1. Tiêu đề Panel
        JLabel lblTitle = new JLabel("QUẢN LÝ DANH SÁCH LAPTOP");
        lblTitle.setFont(new Font("Segoe UI", Font.BOLD, 24));
        lblTitle.setForeground(new Color(45, 52, 54));
        add(lblTitle, "wrap");

        // 2. Thanh công cụ (Nút bấm bo tròn)
        JPanel toolBar = new JPanel(new MigLayout("insets 0", "[grow]10[]10[]"));
        toolBar.setOpaque(false);

        JTextField txtSearch = new JTextField();
        // Placeholder text (Tính năng của FlatLaf)
        ...
        // Ép bo tròn riêng cho nút này (Client Property của FlatLaf)
        ...
        table.setRowHeight(30); // Tăng độ cao dòng cho dễ nhìn
        table.setShowGrid(false); // Ẩn đường kẻ sọc theo style hiện đại
    ...
    public void loadData() {
        ...
        ArrayList<Laptop> list = laptopBUS.getAllLaptops();
